...
error_absolute=zeros(1,numel(tau_range));
error_absolute(:)=inf;
...
for i = 1:numel(tau_range)
    ...
    if(or(curr_error == Inf,curr_error == 0))
        error_absolute(i) = Inf;
    else
        error_absolute(i) = curr_error;
    end
end

%save results
sols.errors=error_absolute;
...
for i = 2:(numel(tau_range))
    if(error_absolute(i)==inf||error_absolute(i-1)==inf)
        ...
        %factor is greater 1.
        error_reduction(i)=error_absolute(i-1)/error_absolute(i);
    end
end
